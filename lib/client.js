...
Client.prototype.putData = function (tableName, primaryKeys, columns, checking, transactionId, callback) {
  ...
  var request = {
    putRowParameter: {
      ...
      rowChange: {
        row: row,
        checkingType: checking ? checking : 'NO'
      }
    }
  };
...
Client.prototype.multiGetRow = function (tableName, pks, columns, callback) {
  ...
  self.request('MultiGetRow', body, function (err, result) {
    ...
    for (var i = 0; i < items.length; i++) {
      var item = items[i];
      item.row = self.parseRow(item.row);
    }
    callback(null, items);
  });
  return this;
...
function (tableName, primaryKeys, primaryKeyRanges, columnNames, top, transactionID, callback) {
  ...
  for (var i = 0, l = primaryKeyRanges.length; i < l; i++) {
    ...
    var begin = this.convertToColumnValue({ Value: range.Begin });
    var end = this.convertToColumnValue({ Value: range.End });
