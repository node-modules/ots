...
Client.prototype.createTableGroup = function (name, partitionKeyType, callback) {
  ...
  // console.log(CreateTableGroupRequest, params, body, body.length, CreateTableGroupRequest.parse(body));
...
Client.prototype.startTransaction = function (entityName, partitionKey, callback) {
  var body = StartTransactionRequest.serialize({
    ...
    partitionKeyValue: this.convert({ Value: partitionKey })
  });
  ...
  this.request('GetRow', params, function (err, result) {
    ...
    if (columns) {
      ...
      for (var i = columns.length; i--;) {
        var col = columns[i];
        if (!col || !col.Value) {
          console.error('OTS parseRows Warnning: row: %j, columns: %j', result.Table.Row, columns);
          row = null;
          break;
        }
  ...
  for (var j = 0, jl = rows.length; j < jl; j++) {
    ...
    if (columns) {
      ...
      for (var i = 0; i < columns.length; i++) {
        var col = columns[i];
        if (!col || !col.Value) {
          console.error('OTS parseRows Warnning: row: %j, columns: %j', item, columns);
          row = null;
          break;
        }
        var parse = Client.ValueParsers[col.Value.type];
